...
import com.hereliesaz.kbox2d.callbacks.ContactImpulse
...
import com.hereliesaz.kbox2d.dynamics.contacts.ContactSolver
import com.hereliesaz.kbox2d.dynamics.contacts.ContactSolver.ContactSolverDef
import com.hereliesaz.kbox2d.dynamics.contacts.ContactVelocityConstraint
...
class Island {
    ...
    lateinit var m_bodies: Array<Body?>
    lateinit var m_contacts: Array<Contact?>
    lateinit var m_joints: Array<Joint?>
    lateinit var m_positions: Array<Position>
    lateinit var m_velocities: Array<Velocity>
    var m_bodyCount = 0
    var m_jointCount = 0
    var m_contactCount = 0
    var m_bodyCapacity = 0
    var m_contactCapacity = 0
    var m_jointCapacity = 0
    fun init(
        ...
        m_jointCount = 0
        m_listener = listener
        if (!::m_bodies.isInitialized || m_bodyCapacity > m_bodies.size) {
            m_bodies = arrayOfNulls(m_bodyCapacity)
        }
        if (!::m_joints.isInitialized || m_jointCapacity > m_joints.size) {
            m_joints = arrayOfNulls(m_jointCapacity)
        }
        if (!::m_contacts.isInitialized || m_contactCapacity > m_contacts.size) {
            m_contacts = arrayOfNulls(m_contactCapacity)
        }

        // dynamic array
        if (!::m_velocities.isInitialized || m_bodyCapacity > m_velocities.size) {
            val old = if (::m_velocities.isInitialized) m_velocities else arrayOf()
            m_velocities = Array(m_bodyCapacity) { Velocity() }
        ...
        // dynamic array
        if (!::m_positions.isInitialized || m_bodyCapacity > m_positions.size) {
            val old = if (::m_positions.isInitialized) m_positions else arrayOf()
            m_positions = Array(m_bodyCapacity) { Position() }
    ...
    fun solve(profile: Profile, step: TimeStep, gravity: Vec2, allowSleep: Boolean) {
        ...
        if (allowSleep) {
            ...
            for (i in 0 until m_bodyCount) {
                ...
                if (b.m_flags and Body.Companion.e_autoSleepFlag == 0 || b.m_angularVelocity * b.m_angularVelocity > angTolSqr || Vec2.dot(
                        b.m_linearVelocity,
                        b.m_linearVelocity
                    ) > linTolSqr
                ) {
