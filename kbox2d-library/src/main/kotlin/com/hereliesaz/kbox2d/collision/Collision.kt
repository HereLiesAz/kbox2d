...
class Collision(private val pool: IWorldPool) {
     ...
     */
    fun testOverlap(
        shapeA: Shape, indexA: Int, shapeB: Shape, i
        ndexB: Int,
        xfA: Transform, xfB: Transform
    ): Boolean {
        input.proxyA.set(shapeA, indexA)
        input.proxyB.set(shapeB, i, ndexB)
    ...
    fun findMaxSeparation(
        results: EdgeResults, poly1: PolygonShape,
    ...
    fun findIncidentEdge(
        c: Array<ClipVertex>, poly1: PolygonShape,
        ...
        fun computePolygonSeparation(axis: EPAxis) {
            ...
            for (i in 0 until m_polygonB.count) {
                ...
                if (n2.x * perp.x + n2.y * perp.y >= 0.0f) {
                    if (Vec2.dot(temp.set(n2).subLocal(m_upperLimit), m_normal) < -Settings.angularSlop) {
                        continue
                    }
                } else {
                    if (Vec2.dot(temp.set(n2).subLocal(m_lowerLimit), m_normal) < -Settings.angularSlop) {
                        continue
                    }
                }
                if (s > axis.separation) {
