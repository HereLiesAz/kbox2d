...
class DynamicTreeFlatNodes : BroadPhaseStrategy {
    ...
    protected lateinit var m_parent: IntArray
    protected lateinit var m_child1: IntArray
    protected lateinit var m_child2: IntArray
    protected lateinit var m_height: IntArray
    ...
    private fun expandBuffers(oldSize: Int, newSize: Int) {
        ...
        m_userData = BufferUtils.reallocateBuffer(Any::class.java, m_userData, oldSize, newSize)
    ...
    override fun query(callback: TreeCallback, aabb: AABB) {
        ...
        while (nodeStackIndex > 0) {
            var node = nodeStack[--nodeStackIndex]
            if (node == NULL_NODE) {
                continue
            }
            if (AABB.testOverlap(m_aabb[node], aabb)) {
                ...
                if (child1 == NULL_NODE) {
                    ...
                    if (nodeStack.size - nodeStackIndex - 2 <= 0) {
                        nodeStack =
                            BufferUtils.reallocateBuffer(nodeStack, nodeStack.size, nodeStack.size * 2)
                    }
    ...
    override fun getAreaRatio(): Float {
        ...
        val root = m_root
        val rootArea = m_aabb[root].getPerimeter()
        var totalArea = 0.0f
        for (i in 0 until m_nodeCapacity) {
            if (m_height[i] < 0) {
                // Free node in pool
    ...
    private fun insertLeaf(leaf: Int) {
        ...
        while (m_child1[index] != NULL_NODE) {
            ...
            val nodeAABB = m_aabb[node]
            val area = nodeAABB.getPerimeter()
            combinedAABB.combine(nodeAABB, leafAABB)
            val combinedArea = combinedAABB.getPerimeter()
            ...
            if (m_child1[child1] == NULL_NODE) {
                ...
                combinedAABB.combine(leafAABB, child1AABB)
                val oldArea = child1AABB.getPerimeter()
                val newArea = combinedAABB.getPerimeter()
            ...
            if (m_child1[child2] == NULL_NODE) {
                ...
                combinedAABB.combine(leafAABB, child2AABB)
                val oldArea = child2AABB.getPerimeter()
                val newArea = combinedAABB.getPerimeter()
    ...
    private fun balance(iA: Int): Int {
        ...
        val C = iC
        val balance = m_height[C] - m_height[iB]
        ...
        if (balance > 1) {
            ...
            // Swap A and C
            m_child1[C] = iA
            val cParent = m_parent[A].also { m_parent[C] = it }
            m_parent[A] = iC
            ...
            // Rotate
            if (m_height[iF] > m_height[iG]) {
                m_child2[C] = iF
                m_child2[A] = iG
                m_parent[iG] = iA
                m_aabb[A].combine(m_aabb[iB], m_aabb[iG])
                m_aabb[C].combine(m_aabb[A], m_aabb[iF])
                m_height[A] = 1 + MathUtils.max(m_height[iB], m_height[iG])
                m_height[C] = 1 + MathUtils.max(m_height[A], m_height[iF])
            } else {
                m_child2[C] = iG
                m_child2[A] = iF
                m_parent[iF] = iA
                m_aabb[A].combine(m_aabb[iB], m_aabb[iF])
                m_aabb[C].combine(m_aabb[A], m_aabb[iG])
                m_height[A] = 1 + MathUtils.max(m_height[iB], m_height[iF])
                m_height[C] = 1 + MathUtils.max(m_height[A], m_height[iG])
            }
            return iC
        }
        ...
        if (balance < -1) {
            ...
            // Swap A and B
            m_child1[iB] = iA
            val Bparent = m_parent[A].also { m_parent[iB] = it }
            m_parent[A] = iB
            ...
            // Rotate
            if (m_height[iD] > m_height[iE]) {
                m_child2[iB] = iD
                m_child1[A] = iE
                m_parent[iE] = iA
                m_aabb[A].combine(m_aabb[C], m_aabb[iE])
                m_aabb[iB].combine(m_aabb[A], m_aabb[iD])
                m_height[A] = 1 + MathUtils.max(m_height[C], m_height[iE])
                m_height[iB] = 1 + MathUtils.max(m_height[A], m_height[iD])
            } else {
                m_child2[iB] = iE
                m_child1[A] = iD
                m_parent[iD] = iA
                m_aabb[A].combine(m_aabb[C], m_aabb[iD])
                m_aabb[iB].combine(m_aabb[A], m_aabb[iE])
                m_height[A] = 1 + MathUtils.max(m_height[C], m_height[iD])
                m_height[iB] = 1 + MathUtils.max(m_height[A], m_height[iE])
            }
            return iB
        }
        return iA
