...
object JavaFXPanelHelper {
     ...
     */
    fun addHelpAndPanelListeners(
        panel: Node, model: TestbedModel,
        controller: AbstractTestbedController, screenDragButton: Int
    ) {
        ...
        panel.setOnZoom { zoomEvent ->
            ...
            val zoom = if (zoomEvent.zoomFactor > 1) TestbedCamera.ZoomType.ZOOM_IN else TestbedCamera.ZoomType.ZOOM_OUT
        ...
        panel.setOnScroll { scrollEvent ->
            ...
            val zoom = if (scrollEvent.deltaY > 1) TestbedCamera.ZoomType.ZOOM_IN else TestbedCamera.ZoomType.ZOOM_OUT
            currTest.camera.zoomToPoint(mouse, zoom)
        }
        panel.setOnMouseReleased { mouseEvent: MouseEvent ->
            if (toInt(mouseEvent.button) == screenDragButton) {
                screenDragButtonDown = false
            } else if (model.codedKeys[toInt(KeyCode.SHIFT)] && !mouseJointButtonDown) {
                controller.queueMouseUp(toVec(mouseEvent), TestbedTest.BOMB_SPAWN_BUTTON)
            } else {
                if (toInt(mouseEvent.button) == TestbedTest.MOUSE_JOINT_BUTTON) {
                    mouseJointButtonDown = false
                }
                controller.queueMouseUp(
                    Vec2(mouseEvent.x.toFloat(), mouseEvent.y.toFloat()),
                    toInt(mouseEvent.button)
                )
            }
        }
        panel.setOnMousePressed { mouseEvent: MouseEvent ->
            if (toInt(mouseEvent.button) == screenDragButton) {
                ...
                controller.queueMouseDown(toVec(mouseEvent), toInt(mouseEvent.button))
            }
        }
        panel.setOnMouseMoved { mouseEvent: MouseEvent ->
            mouse.set(toVec(mouseEvent))
            controller.queueMouseMove(toVec(mouseEvent))
        }
        panel.setOnMouseDragged { mouseEvent: MouseEvent ->
            mouse.set(toVec(mouseEvent))
            if (screenDragButtonDown) {
                ...
                controller.queueMouseDrag(Vec2(mouse), TestbedTest.MOUSE_JOINT_BUTTON)
            } else if (model.codedKeys[toInt(KeyCode.SHIFT)]) {
                controller.queueMouseDrag(toVec(mouseEvent), TestbedTest.BOMB_SPAWN_BUTTON)
            } else {
                controller.queueMouseDrag(toVec(mouseEvent), toInt(mouseEvent.button))
            }
        }
